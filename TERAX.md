...
Unavailable roots remain unavailable until the user explicitly recovers them with a valid folder. Local and WSL environments are fixed when a Space is created, and selecting another environment creates a new Space after explicit folder selection. Space activation applies the target environment before exposing its root. Authorization for inactive terminal cwd activity uses the terminal's owning Space environment. Deleting a Space activates a valid fallback through the same controller.
