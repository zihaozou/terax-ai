...
- **tabs/** - `useTabs` is the source of truth for tab list + active id. Each tab records its owning Space; `basename` splits on both `/` and `\`.
...
- **statusbar/** - bottom bar, `CwdBreadcrumb` (complete Unix/Windows/Home/UNC path, ancestor root changes, trailing environment-scoped child-directory `...`, horizontal wheel scrolling, right-edge auto-scroll, and stale-request rejection via tested `pathUtils` helpers).
...
- **spaces/** - workspace spaces/projects (name, root, env, color, per-space tab persistence) via `useSpaces`, `SpaceSwitcher`, and the serialized Space controller.

### Space-owned workspace context

`SpaceMeta.root` and `SpaceMeta.env` are the only workspace-level authority. A Space root is stable across tab activation, pane focus, external file opens, and OSC 7 updates. Terminal and pane cwd values are runtime-only shell state, and OSC 7 never changes a Space root.

The bottom status bar is the only root-mutation UI. Changing a root affects Explorer, Source Control, and new terminals for the active Space, but never sends input to or changes an existing terminal. Explorer and Source Control always consume the active Space root; Source Control has no fixed repository target. Files outside that root remain openable in their owning Space.

First boot and generic `New Space` use the fixed environment's Home as root; the first cold terminal uses the same Home, and generic spaces receive a collision-free `Space N` name. The bottom breadcrumb is the only root picker: ancestors move upward and the trailing `...` lists direct children for one-level downward navigation using the Space environment explicitly. It has no system folder picker. An unavailable persisted root automatically recovers to the canonical, authorized Home of the same environment; a failed WSL Home never substitutes Local Home. Root issues pass `null` to every workspace consumer even when the rejected persisted string remains non-null. Failed active-environment adoption leaves persisted tabs unmounted, shows one rootless cold fallback, and disables tab-state persistence for that session so the fallback cannot overwrite the saved snapshot. Space activation applies the target environment before exposing its root. Authorization for restored and live terminal cwd activity uses the terminal's owning Space environment. Rust owns canonical display paths, including UNC prefixes and legal Unix backslashes. Deleting a Space activates a valid fallback through the same controller.
